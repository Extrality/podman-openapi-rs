...
**hosts_file** | Option<**String**> | HostsFile is the base file to create the `/etc/hosts` file inside the infra container which will be shared with the rest of the pod. | [optional]
...
**no_manage_hostname** | Option<**bool**> | NoManageHostname indicates that the pod will not manage /etc/hostname and instead each container will handle their own. | [optional]
